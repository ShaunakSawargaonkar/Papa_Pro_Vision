    ...
    return MaterialApp(
      ...
      home: StreamBuilder<User?>(
        ...
        builder: (context, authSnapshot) {
          ...
          // user = null;
          if (user == null || user.phoneNumber == null || user.uid.isEmpty || user.phoneNumber!.isEmpty) {
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData) {
                ...
                if (data.userStatus == UserStatus.errorState) {
                  ...
                  return RegistrationPage(phoneNumber: user?.phoneNumber ?? '', userUID: user?.uid ?? '');
                } else if (data.userStatus == UserStatus.firstPaymentPending) {
                  return PaymentGateway(isFirstPayment: true, userUID: user?.uid ?? '', phoneNumber: user?.phoneNumber ?? '');
                } else if (data.userStatus == UserStatus.paymentPending) {
                  return PaymentGateway(isFirstPayment: false, userUID: user?.uid ?? '', phoneNumber: user?.phoneNumber ?? '');
                  ...
                  return AlasPage(
                    ...
                    message: data.message ?? 'Your app version is not supported. Please update to the latest version.',
                  );
                }
              }
              return AlasPage(
